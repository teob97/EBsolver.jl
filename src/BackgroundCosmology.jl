export BackgroundCosmologySetup, BackgroundCosmology
export eval_ρ0_crit
...
Base.@kwdef struct BackgroundCosmologySetup
...
dHpdx_of_x(BC::BackgroundCosmologySetup, x::AbstractRange)		= dHpdx_of_x(BC, collect(x))

ddHpddx_of_x(BC::BackgroundCosmologySetup, x::Float64)			= Hp_of_x(BC, x) * 0.5 * BC.H0_SI^2 * (2*(BC.Ω0_B+BC.Ω0_CDM)*exp(-3x) + 6*(BC.Ω0_γ+BC.Ω0_nu)*exp(-4x) + 2*BC.Ω0_Λ)
...
function comoving_distance_of_x(BC::BackgroundCosmologySetup, x::Union{Float64, Vector{Float64}})
	η = η_of_x(BC)
	# Rember that we are working with x=log(a) where a is the scale factor
	return η(log(1.0)) .- η.(x)
end
function comoving_distance_of_x(BC::BackgroundCosmologySetup, x::AbstractRange)
	return comoving_distance_of_x(BC, collect(x))
end

function transverse_comoving_distance_of_x(BC::BackgroundCosmologySetup, x::Union{Float64, Vector{Float64}})
...
function angular_distance_of_x(BC::BackgroundCosmologySetup, x::Union{Float64, Vector{Float64}}) 
	r = transverse_comoving_distance_of_x(BC, x)
	return exp.(x) .* r
end
function angular_distance_of_x(BC::BackgroundCosmologySetup, x::AbstractRange)
	return angular_distance_of_x(BC, collect(x))
end

function luminosity_distance_of_x(BC::BackgroundCosmologySetup, x::Union{Float64, Vector{Float64}})
	r = transverse_comoving_distance_of_x(BC, x)
	return exp.(-x) .* r
end
function luminosity_distance_of_x(BC::BackgroundCosmologySetup, x::AbstractRange)
